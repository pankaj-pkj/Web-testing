...
<div id="fa">
  <div class="card">
    ...
    <button class="btn" id="sb" onclick="go()">⚡ LAUNCH PHANTOM v4.0</button>
...
async function go(){
  ...
  document.getElementById('sb').disabled=true;document.getElementById('sb').textContent='SCANNING...';
  document.getElementById('fa').style.display='none';document.getElementById('pa').style.display='block';
  document.getElementById('tm').innerHTML='';li=0;es=0;
  const phg=document.getElementById('phg');phg.innerHTML='';
  PH.forEach(p=>{const d=document.createElement('div');d.className='ph';
    d.innerHTML='<div class="ph-n">'+p+'</div><div class="ph-t"><div class="ph-b" id="phb_'+p.replace(/[^A-Za-z0-9]/g,'')+'"></div></div>';phg.appendChild(d);});
  et=setInterval(()=>{es++;document.getElementById('pm').textContent=es+'s — scanning...'},1000);
  const fd=new FormData();fd.append('url',url);
  const r=await fetch('/scan',{method:'POST',body:fd});const d=await r.json();sid=d.scan_id;
  ...
  if(cnt===0){vl.innerHTML='<div class="nv">✅ No vulnerabilities detected — site appears secure for these tests.</div>';}
    ...
    vs.forEach((v,i)=>{
      ...
      if(v.fix&&v.fix.length){
        h+='<button class="ftb" onclick="tf(''+fi+'')">▸ Remediation Steps</button>'+
           '<div class="fd" id="'+fi+'"><ol>';
        v.fix.forEach(step=>h+='<li>'+esc(step)+'</li>');
        h+='</ol></div>';}
